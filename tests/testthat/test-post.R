...
test_that("post_toot, defensive", {
  ...
  ## super long alttext
  expect_error(post_toot("testing in progress, please ignore",
...
test_that("post_toot, real", {
  vcr::use_cassette("post_toot_default", {
    expect_error(x <- post_toot(status = "testing in progress, please ignore", token = fake_token, verbose = FALSE), NA)
  })
  vcr::use_cassette("post_toot_media", {
    expect_error(x <- post_toot(
      status = "testing in progress, please ignore", media = "../testdata/logo.png", alt_text = "rtoot logo",
      token = fake_token, verbose = FALSE
    ), NA)
  })
  vcr::use_cassette("post_toot_spoiler_text", {
    expect_error(x <- post_toot(
      status = "testing in progress, please ignore", media = "../testdata/logo.png", alt_text = "rtoot logo",
      spoiler_text = "rtoot is the best", token = fake_token, verbose = FALSE
    ), NA)
  })
  vcr::use_cassette("post_toot_sensitive", {
    expect_error(x <- post_toot(
      status = "testing in progress, please ignore", media = "../testdata/logo.png", alt_text = "rtoot logo",
      spoiler_text = "rtoot is the best", sensitive = TRUE, token = fake_token, verbose = FALSE
    ), NA)
  })
  vcr::use_cassette("post_toot_visibility", {
    expect_error(x <- post_toot(
      status = "testing in progress, please ignore", media = "../testdata/logo.png", alt_text = "rtoot logo",
      spoiler_text = "rtoot is the best", visibility = "unlisted", token = fake_token, verbose = FALSE
    ), NA)
  })
  vcr::use_cassette("post_toot_language", {
    expect_error(x <- post_toot(
      status = "jetzt testen", media = "../testdata/logo.png", alt_text = "rtoot logo",
      language = "de", visibility = "unlisted", token = fake_token, verbose = FALSE
    ), NA)
  })
})
...
test_that("post_user", {
  ...
  vcr::use_cassette("post_user_pin", {
    ...
    expect_error(post_user("5358", action = "pin", token = fake_token, verbose = FALSE), NA)
  })
})
...
test_that("post_toot, verbose", {
  vcr::use_cassette("post_toot_verbose", {
    expect_message(x <- post_toot(status = "testing in progress, please ignore", token = fake_token2, verbose = TRUE))
  })
  vcr::use_cassette("post_toot_silent", {
    expect_silent(x <- post_toot(status = "testing in progress, please ignore", token = fake_token2, verbose = FALSE))
  })
})
...
test_that("post_user silent", {
  vcr::use_cassette("post_user_pin_silent", {
    ...
    expect_silent(post_user("109337044410360200", action = "pin", token = fake_token2, verbose = FALSE))
  })
})
