...
auth_setup <- function(
  ...
  while (is.null(instance) || instance == "") {
    instance <- rtoot_ask(
      prompt = "On which instance do you want to authenticate (e.g., \"mastodon.social\")? ",
      pass = FALSE
    )
  }
  ...
  if (!isTRUE(type %in% c("public", "user"))) {
    type <- c("public", "user")[rtoot_menu(
      choices = c("public", "user"),
      title = "What type of token do you want?",
      verbose = TRUE
    )]
  }
  token <- process_created_token(
    create_token(client, type = type, browser = browser),
    name = name,
    path = path,
    clipboard = clipboard,
    verify = TRUE,
...
process_created_token <- function(
  token,
  name = NULL,
  path = NULL,
  clipboard = FALSE,
...
get_token_from_envvar <- function(
  envvar = "RTOOT_DEFAULT_TOKEN",
  check_stop = TRUE
) {
  dummy <- list(bearer = "")
  dummy$type <- ""
