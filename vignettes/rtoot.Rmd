...
For example, some calls can only be made within your own instance (e.g `get_home_timeline()`), others can access all instances but you need to specify the instance as a parameter (e.g. `get_public_timeline()`). 
...
To get the most recent toots of a specific instance use `get_public_timeline()`
```{r, eval = FALSE}
get_public_timeline(instance = "mastodon.social")
```

To get the most recent toots containing a specific hashtag use `get_hashtag_timeline()`

```{r, eval = FALSE}
get_hashtag_timeline(hashtag = "rstats", instance = "mastodon.social")
```

The function `get_home_timeline()` allows you to get the most recent toots from your own timeline.
